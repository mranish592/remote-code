...
async function executeJavascript(code, codeFileName) {
    ...
    try {
        ...
        console.log("Directory created successfully at", ServerConfig.CODE_FILES_PATH);

        // Write to a file
        await fs.writeFile(`${ServerConfig.CODE_FILES_PATH}/${codeFileName}.js`, code, "utf8");
        console.log("File write successful at", `${ServerConfig.CODE_FILES_PATH}/${codeFileName}.js`);
        ...
        const { stdout, stderr } = await execAsync(
            `bash ${scriptPath} ${ServerConfig.CODE_FILES_PATH}/${codeFileName}.js ${ServerConfig.CODE_FILES_PATH}/${codeFileName}.txt`
        );
